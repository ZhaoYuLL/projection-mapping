...
def generate_d_j_array():
    ...
    for i in range(len(arr_t_j)):
        # a = d_j(target_points[0].get('x') - arr[i].get('x'),target_points[0].get('y') - arr[i].get('y'),target_points[0].get('z') - arr[i].get('z'))
        # print(i)
        
        #TODO change target_points[0] to target_points[i]
        
        a = ((target_points[0].get('x') - arr_t_j[i].get('x'))**2  + (target_points[0].get('y') - arr_t_j[i].get('y'))**2  + (target_points[0].get('z') - arr_t_j[i].get('z'))**2)**0.5
        objs.append(a)

    return objs
array_d_j= generate_d_j_array()
# print(f"d_j: {array_d_j}")
# print(len(array_d_j))
# print("\n")
...
def generate_e_j_array():
    objs = list()
    for i in range(len(arr_t_j)):
        a = d_j(target_points[0].get('x') - arr_t_j[i].get('x'),target_points[0].get('y') - arr_t_j[i].get('y'),target_points[0].get('z') - arr_t_j[i].get('z'))
        objs.append(a)
    return (tuple(vars(a) for a in objs))
...
def generate_sinj_array():
    objs  = list()
    for i in range(len(arr_e_j)):
        a = ((arr_e_j[i].get('x') ** 2 + arr_e_j[i].get('y') ** 2)/(arr_e_j[i].get('x') ** 2 + arr_e_j[i].get('y') ** 2+ arr_e_j[i].get('z') ** 2)) ** 0.5
        objs.append(a)
    return objs
arr_sinj= generate_sinj_array()
# print(f"sinj: {arr_sinj}")
# print(len(arr_sinj))
# print("\n")
# print(f"test i*k {i*k}")
# print(f"test i*k*d_j_1 {i*k*array_d_j[0]}")

# print(f"test exp: {cmath.exp(i*k*array_d_j[255])}")
# c = j_0(0,k*r*arr_sinj[2])
# print(f"cccc: {c}")
# print(f"TEST: {(p_0 * c * cmath.exp(i*k*array_d_j[3]))/(array_d_j[3])}")
# print(f"max d_j: {max(array_d_j)}")
...
# print(f"pressure_array: {arr_p_j}")
#print("\n")
def sum_of_pj_array():
    for j in range(256):
